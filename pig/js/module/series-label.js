	...
	(function(n) {
				...
				if (this.getPointSpline && f.getPointAtLength && !q) {
					...
					for (h = d.length, c = 0; c < h; c += 1) {
						...
						if (0 < c && (e = Math.abs(b.chartX -
							...
							for (e = Math.ceil(e / 16), f = 1; f < e; f += 1) m.push({
		...
		z.prototype.checkClearPoint = function(d, b, a, m) {
			var c =
				Number.MAX_VALUE,
				e = Number.MAX_VALUE,
				f, h, g = this.options.label.connectorAllowed,
				q = x(this.options.label.onArea, !!this.area),
				r = this.chart,
				u, k, n, t, p, l;
			for (p = 0; p < r.boxesToAvoid.length; p += 1)
				if (k = r.boxesToAvoid[p], l = d + a.width, u = b, n = b + a.height, !(d > k.right || l < k.left || u > k.bottom || n < k.top)) return !1;
				...
				if (a.xAxis || a.yAxis) {
					...
					if (a.visible && !a.isSeriesBoosting && p) {
						r || (a.labelBySeries = r = d.renderer.label(a.name, 0, -9999, "connector").css(E({
							color: l ? d.renderer.getContrast(a.color) : a.color
						...
						if (!h.length && !l)
							for (e = n + t - c.width; e >= n; e -= 16)
								for (f = k; f < k + v - c.height; f += 16)(g = a.checkClearPoint(e, f, c, !0)) && h.push(g);
						if (h.length) {
